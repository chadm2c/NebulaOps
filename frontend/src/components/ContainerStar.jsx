...
const fragmentShader = `
  ...
  void main() {
    ...
    if (uIsStopped > 0.5) {
      baseColor = vec3(0.1, 0.1, 0.12);
    }
    
    if (uIsPaused > 0.5) {
      float amberPulse = sin(uTime * 0.8) * 0.5 + 0.5;
      baseColor = mix(vec3(0.15, 0.12, 0.05), vec3(0.7, 0.4, 0.1), amberPulse);
    }
...
function ContainerStar({ container, position, onSelect, selectedId, onWarpTo, onToggleConstellation, highlightedNetwork, onOpenBridge }) {
  ...
  const uniforms = useMemo(() => ({
    ...
    uColorError: { value: new THREE.Color('#ff2222') },
    uGlow: { value: isRunning ? 1 : 0 },
    uIsPaused: { value: isPaused ? 1 : 0 },
    uIsStopped: { value: isStopped ? 1 : 0 }
  }), [])
  ...
  useFrame((state) => {
    ...
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.uIsPaused.value = isPaused ? 1 : 0
      materialRef.current.uniforms.uIsStopped.value = isStopped ? 1 : 0
      materialRef.current.uniforms.uGlow.value = isRunning ? 1 : 0
      ...
      if (meshRef.current) {
        let scale = size
        if (isPaused) {
          scale *= (1 + Math.sin(elapsed * 1.5) * 0.15)
        } else if (!isStopped) {
          scale *= (1 + breath * 0.15)
        }
    ...
    if (glowRef.current && !isStopped) {
      const glowScale = isPaused 
        ? 1.5 + Math.sin(elapsed * 1.5) * 0.2
        : 1.5 + Math.sin(elapsed * 3) * 0.1
      glowRef.current.scale.setScalar(size * glowScale)
    }
  })
  
  const color = useMemo(() => {
    if (isStopped) return new THREE.Color('#1a1a1f')
    if (isPaused) return new THREE.Color('#ffaa00')
    ...
    <group position={position}>
      ...
      >
        ...
        <shaderMaterial
          ...
          uniforms={uniforms}
          transparent
          depthWrite={isStopped ? true : false}
        />
      </mesh>
      ...
      {!isStopped && (
        <pointLight
          color={color}
          intensity={isError ? 3 : isPaused ? 2 : 1.5}
          distance={isPaused ? 8 : 12}
          decay={2}
        />
      )}
      
      {/* HUD Container overlay. Used instead of standard HTML label when selected, or standard title when not */}
      <Html
        position={[0, 0, 0]}
        center
        zIndexRange={[100, 0]}
          ...
          <AnimatePresence>
            {selectedId === container.id ? (
              ...
              <div style={{
                ...
                whiteSpace: 'nowrap',
                pointerEvents: 'none',
                transform: `translateY(${-(size * 20 + 30)}px)`,
                opacity: 0.7,
